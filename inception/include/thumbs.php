...
}  //42 Define if a png image should have a transparent background color. Use False value if you want to display a custom coloured canvas_colour
...
class timthumb
{
    ...
    public function __construct()
    {
        ...
        if (BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER', $_SERVER) && (!preg_match('/^https?:\/\/(?:www\.)?' . $this->myHost . '(?:$|\/)/i', \Xmf\Request::getString('HTTP_REFERER', '', 'SERVER')))) {
            ...
            header('Content-Length: ' . count($imgData));
        ...
        if ($this->isURL) {
            if (ALLOW_ALL_EXTERNAL_SITES) {
                ...
                foreach ($ALLOWED_SITES as $site) {
                    if ((mb_strtolower(mb_substr($this->url['host'], -mb_strlen($site) - 1)) === mb_strtolower(".$site")) || (mb_strtolower($this->url['host']) === mb_strtolower($site))) {
    ...
    protected function processImageAndWriteToCache($localImage)
    {
        ...
        if (3 == mb_strlen($canvas_color)) { //if is 3-char notation, edit string into 6-char notation
    ...
    protected function getLocalImagePath($src)
    {
        ...
        if (mb_strstr($_SERVER['SCRIPT_FILENAME'], ':')) {
